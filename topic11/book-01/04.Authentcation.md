...
Currently movies and profile are not protected. We need to create a <PrivateRoute > component to  force authentication using the login page.
...
This component uses the AuthContext to check if a user is authenticated. If not, a <redirect > component to the login page is returned 
...
+ In the */src* folder, create a file called authContext.js and add the following content:
