...
On the Home page, we can tag a movie as one of our favourites by clicking its 'Favourite' icon. This tagging will cause the related card to show a red Favorite Icon on the title bar. As a temporary solution, we will record a tagging by adding a 'favourite' property to the movie object and set it to true. But realistically, they should be persisted to a database via an API. 
...
~~~
    ...
    />
~~~
So far, a component prop is either a data object or a callback functions. However, a component is the value assigned to the title and avatar props above. This is because a component transpiles to a function. Later we will develop custom components that accept other components as props.
...
with this:
~~~
    <IconButton aria-label="add to favourites" onClick={handleAddToFavuorite}>
        <FavoriteIcon color="primary" fontSize="large" />
    </IconButton>
~~~
...
~~~
We must update the routing configuration and add a simple site navigation bar. In `src/index.js`, replace the App component as illustrated below:
~~~
.... other imports .....
