...
- Create a new file in your application root folder(*node-lab1*) called **.env** and add the following content:
~~~javascript
...
- Open the *.gitignore* file in you *wad-api-labs-2020* folder and confirm that it contains the following entry(it should be there with other entries from when you set up the repo in the previous steps):
~~~json
.env
~~~
...
Finally we call ``server.listen(config.port)`` to tell node.js the port on which we want our HTTP server to run.
...
- Change ``index.js`` to return "Hello WAD2!" in the response and take a look at the terminal. You should see the console output indicating a restart. However, you will require a refresh in the browser to see the new "Hello WAD2!" message.